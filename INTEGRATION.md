...
Сделано. В `internal/api/server.go` было:
...
Импорт `internal/webui` убран вместе с функцией `staticHandler`, которая
обслуживала только его. Сам пакет оставлен на один релиз — но не
смонтирован: два хендлера на `/` несовместимы, `ServeMux` паникует на
дублирующемся паттерне.
...
Сделано, но не так, как предлагалось ниже. Next кладёт в страницу инлайн-
скрипты с данными для гидратации — в экспорте их около двух десятков на
страницу, а на странице 404 есть ещё и инлайн-стиль. Под `script-src 'self'`
они блокируются: страница откроется, но ни одна кнопка не заработает.

Простой вариант — разрешить `'unsafe-inline'`, то есть любой инлайн-скрипт,
включая внедрённый через XSS. Вместо этого `internal/web` считает sha256
каждого инлайн-скрипта и инлайн-стиля **при старте процесса, из уже вшитой
статики**, и дописывает хэши в директивы `script-src` и `style-src` того CSP,
который выставил `securityHeaders`. Политика по-прежнему объявлена в одном
месте — `internal/api/server.go`; хендлер сайта её только расширяет на
конкретной странице и ничего не делает, если заголовок не выставлен.

Обычное возражение против хэшей — «забыли пересчитать после пересборки» —
здесь не работает: считается не на этапе сборки, а из тех самых байтов,
которые отдаются клиенту. Пересобрали сайт — пересчитались и хэши.
