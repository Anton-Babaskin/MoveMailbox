...
- **Автоопределение языка** (`web/public/lang.js`, подключён до первой
  отрисовки). Правила узкие намеренно: перенаправляем только с русской версии
  (она же x-default), только один раз, путь сохраняем; адрес с префиксом не
  трогаем; явный выбор в переключателе запоминается в `mm.lang` и отменяет
  автоопределение навсегда; поисковых роботов не трогаем, чтобы каждая языковая
  версия доставалась им по своему адресу, как объявлено в hreflang.
  Проверено в браузере: ru → `/`, en → `/en/`, uk → `/uk/`, de → `/` (x-default),
  `/routes/` → `/en/routes/`, уже-префиксный адрес без изменений, после явного
  выбора RU перенаправления нет, Googlebot не перенаправляется.
...
- **Страница 404 чинилась дважды.** Сначала обнаружилось, что в `404.html`
  ...
  `experimental.globalNotFound`: страница рендерит документ сама. Заодно снят
